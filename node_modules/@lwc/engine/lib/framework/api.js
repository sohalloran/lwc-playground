...
const ElementHook = {
    ...
    destroy: hooks_1.destroyElmHook,
};
const CustomElementHook = {
    ...
    update: (oldVnode, vnode) => {
        ...
        hooks_1.renderCustomElmHook(vnode);
    },
    insert: (vnode, parentNode, referenceNode) => {
        node_1.insertBefore.call(parentNode, vnode.elm, referenceNode);
        hooks_1.createChildrenHook(vnode);
    ...
    destroy: (vnode) => {
        hooks_1.destroyCustomElmHook(vnode);
        hooks_1.destroyElmHook(vnode);
    },
};
...
function getCurrentOwnerId() {
    ...
    return invoker_1.vmBeingRendered.uid;
}
const getCurrentFallback = dom_1.isNativeShadowRootAvailable ?
    function () {
        if (process.env.NODE_ENV !== 'production') {
            // TODO: enable this after refactoring all failing tests
            // assert.invariant(!isNull(vmBeingRendered), `Invalid invocation of getCurrentFallback().`);
        }
        return invoker_1.vmBeingRendered.fallback;
    } : () => {
    if (process.env.NODE_ENV !== 'production') {
        // TODO: enable this after refactoring all failing tests
        // assert.invariant(!isNull(vmBeingRendered), `Invalid invocation of getCurrentFallback().`);
...
function getCurrentShadowAttribute() {
    ...
    return invoker_1.vmBeingRendered.context.shadowAttribute;
}
// [h]tml node
function h(sel, data, children) {
    if (process.env.NODE_ENV !== 'production') {
        ...
        language_1.forEach.call(children, (childVnode) => {
            if (childVnode != null) {
                assert_1.default.isTrue(childVnode && "sel" in childVnode && "data" in childVnode && "children" in childVnode && "text" in childVnode && "elm" in childVnode && "key" in childVnode, `${childVnode} is not a vnode.`);
            }
        });
    }
    const { key } = data;
    ...
    if (sel.length === 3 && language_1.StringCharCodeAt.call(sel, 0) === CHAR_S && language_1.StringCharCodeAt.call(sel, 1) === CHAR_V && language_1.StringCharCodeAt.call(sel, 2) === CHAR_G) {
        addNS(vnode);
    }
    return vnode;
}
...
function s(slotName, data, children, slotset) {
    ...
    if (!language_1.isUndefined(slotset) && !language_1.isUndefined(slotset[slotName]) && slotset[slotName].length !== 0) {
...
function gid(id) {
    ...
    return language_1.isNull(id) ? id : `${id}-${getCurrentOwnerId()}`;
}
exports.gid = gid;
