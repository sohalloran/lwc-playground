    ...
    return {
        ...
        get() {
            ...
            if (invoker_1.isBeingConstructed(vm)) {
                if (process.env.NODE_ENV !== 'production') {
                    assert_1.default.logError(`${vm} constructor should not read the value of property "${propName}". The owner component has not yet set the value. Instead use the constructor to set default values for properties.`, vm.elm);
                }
                return;
            }
...
function BaseLightningElement() {
    ...
    if (process.env.NODE_ENV !== 'production') {
        assert_1.default.isTrue(invoker_1.vmBeingConstructed && "cmpRoot" in invoker_1.vmBeingConstructed, `${invoker_1.vmBeingConstructed} is not a vm.`);
    ...
    vm_1.setNodeKey(elm, uid);
    if (process.env.NODE_ENV !== 'production') {
        restrictions_2.patchComponentWithRestrictions(component, utils_1.EmptyObject);
...
BaseLightningElement.prototype = {
    ...
    dispatchEvent(event) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            if (vm.idx === 0) {
                assert_1.default.logWarning(`Unreachable event "${evtName}" dispatched from disconnected element ${component_1.getComponentAsString(this)}. Events can only reach the parent element after the element is connected (via connectedCallback) and before the element is disconnected(via disconnectedCallback).`, elm);
            }
            if (!evtName.match(/^[a-z]+([a-z0-9]+)?$/)) {
                assert_1.default.logWarning(`Invalid event type "${evtName}" dispatched in element ${component_1.getComponentAsString(this)}. Event name should only contain lowercase alphanumeric characters.`, elm);
            }
        }
    ...
    addEventListener(type, listener, options) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            assert_1.default.invariant(language_1.isFunction(listener), `Invalid second argument for this.template.addEventListener() in ${vm} for event "${type}". Expected an EventListener but received ${listener}.`);
        }
...
if (process.env.NODE_ENV !== 'production') {
    restrictions_1.patchLightningElementPrototypeWithRestrictions(BaseLightningElement.prototype, utils_1.EmptyObject);
}
