...
function createStyleElement(styleContent) {
    const elm = document_1.createElement.call(document, 'style');
    elm.type = 'text/css';
...
function getCachedStyleElement(styleContent) {
    ...
    if (language_1.isUndefined(fragment)) {
        fragment = document_1.createDocumentFragment.call(document);
        const elm = createStyleElement(styleContent);
        node_1.appendChild.call(fragment, elm);
...
function insertGlobalStyle(styleContent) {
    ...
    if (language_1.isUndefined(InsertedGlobalStyleContent[styleContent])) {
        ...
        const elm = createStyleElement(styleContent);
        node_1.appendChild.call(globalStyleParent, elm);
    }
}
function noop() {
    /** do nothing */
...
function evaluateCSS(vm, stylesheets, hostAttribute, shadowAttribute) {
    ...
    if (fallback) {
        ...
        return language_1.forEach.call(stylesheets, stylesheet => {
        ...
        const textContent = language_1.ArrayReduce.call(stylesheets, (buffer, stylesheet) => {
            return buffer + stylesheet(language_1.emptyString, language_1.emptyString, true);
        }, '');
