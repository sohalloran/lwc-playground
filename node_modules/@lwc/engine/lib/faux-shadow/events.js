...
function getRootNodeHost(node, options) {
    let rootNode = traverse_1.getRootNodeGetter.call(node, options);
...
function attachDOMListener(elm, type, wrappedListener) {
    ...
    if (cmpEventHandlers.length === 0) {
        element_1.addEventListener.call(elm, type, domListener);
    }
...
function detachDOMListener(elm, type, wrappedListener) {
    ...
    let p;
    let listeners;
    if (!language_1.isUndefined(listeners = listenerMap[type]) && (p = language_1.ArrayIndexOf.call(listeners, wrappedListener)) !== -1) {
        language_1.ArraySplice.call(listeners, p, 1);
        // only remove from DOM if there is no other listener on the same placement
        if (listeners.length === 0) {
            element_1.removeEventListener.call(elm, type, domListener);
        }
    }
    else if (process.env.NODE_ENV !== 'production') {
...
function addCustomElementEventListener(elm, type, listener, options) {
    if (process.env.NODE_ENV !== 'production') {
        assert_1.default.invariant(language_1.isFunction(listener), `Invalid second argument for this.template.addEventListener() in ${language_1.toString(elm)} for event "${type}". Expected an EventListener but received ${listener}.`);
        // TODO: issue #420
