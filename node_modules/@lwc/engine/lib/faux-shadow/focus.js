...
function getTabbableSegments(host) {
    ...
    const inner = element_1.querySelectorAll.call(host, TabbableElementsQuery);
...
function getActiveElement(host) {
    ...
    return (node_1.compareDocumentPosition.call(host, activeElement) & node_1.DOCUMENT_POSITION_CONTAINED_BY) !== 0 ? activeElement : null;
}
...
function keyboardFocusInHandler(event) {
    ...
    (language_1.isFalse(isFirstFocusableChildReceivingFocus) && language_1.isFalse(isLastFocusableChildReceivingFocus)) ||
...
function willTriggerFocusInEvent(target) {
    return (target !== document_1.DocumentPrototypeActiveElement.call(document) && // if the element is currently active, it will not fire a focusin event
        isFocusable(target));
}
...
function handleFocusMouseDown(evt) {
    ...
    if (willTriggerFocusInEvent(target)) {
        element_1.addEventListener.call(dom_1.eventCurrentTargetGetter.call(evt), 'focusin', stopFocusIn, true);
    }
}
...
function ignoreFocusIn(elm) {
    if (process.env.NODE_ENV !== 'production') {
        assert_1.default.invariant(element_1.tabIndexGetter.call(elm) !== -1, `Invalid attempt to ignore focus in  ${language_1.toString(elm)}. ${language_1.toString(elm)} should not have tabIndex -1`);
    }
